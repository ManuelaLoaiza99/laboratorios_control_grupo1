...
class NodePosition():
	...
	self.phi = [np.pi/6]
	self.pos = 0
	self.xd = 2*np.cos(np.linspace(-2*np.pi,2*np.pi,len(self.t)+1))
	self.yd = 2*np.sin(np.linspace(-2*np.pi,2*np.pi,len(self.t)+1))
	self.xr = list([self.xc[0] + 0.4*np.cos(self.phi[0])])
	self.yr = list([self.yc[0] + 0.4*np.sin(self.phi[0])])
    ...
    def controller(self):
        self.xe = self.xd[self.pos] - self.xr
        self.ye = self.yd[self.pos] - self.yr
	if abs(self.xe) <= 1 and abs(self.ye) <= 1:
        ...
        self.h = [[self.xd[self.pos+1] - self.xd[self.pos]],
                      [self.yd[self.pos+1] - self.yd[self.pos]]]

...
class NodePosition():
	...
	self.yc = 0
	self.phi = np.pi/6
	self.xd = 6*np.cos(np.linspace(-2*np.pi,2*np.pi,len(self.t)+1)
	self.yd = 6*np.sin(np.linspace(-2*np.pi,2*np.pi,len(self.t)+1)
	self.xr = self.xc + 0.3*np.cos(self.phi)
	self.yr = self.yc + 0.3*np.sin(self.phi)
	self.pos = 0
